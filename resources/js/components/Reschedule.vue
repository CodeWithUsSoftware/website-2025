    ...
    <div class="max-w-5xl bg-white rounded-2xl mx-auto shadow-card py-10 overflow-x-hidden" v-if="showForm">
          ...
          <ol role="list" class="bg-white rounded-md shadow px-6 flex space-x-4">
            <li class="flex">
              <div class="flex items-center">
                <a href="#reschedule" @click="step = 1" class="text-gray-400 hover:text-gray-500 cursor-pointer" :class="step === 1 ? ' py-2' : ''">
                  <div v-if="step === 1">
                    <img
                      :class="step === 1 ? 'bg-primary hover:bg-primary-darker-1 w-20 h-20' : 'bg-gray-500 hover:bg-gray-700'"
                      width="24" height="24" src="/images/trial_form/monthly_form.webp" alt="">
                    <span class="sr-only">Home</span>
                  </div>
                  <div v-else>
                    <a @click="step = 1" href="#reschedule" class="ml-4 text-sm"
            ...
            <li class="flex" v-if="step > 1">
              <div class="flex items-center">
                ...
                <a @click="step = 2" href="#reschedule" class="ml-4 text-sm"
            ...
            <li class="flex" v-if="step > 2">
              <div class="flex items-center">
                ...
                <a @click="step = 3" :class="step === 3 ? 'text-primary hover:text-primary-darker-1' : 'text-gray-500 hover:text-gray-700'"
            ...
            <li class="flex" v-if="step > 3 && !reschedule.cancel">
              <div class="flex items-center">
                ...
                <a @click="step = 4" :class="step === 4 ? 'text-primary hover:text-primary-darker-1' : 'text-gray-500 hover:text-gray-700'"
            ...
            <li class="flex" v-if="step > 4">
              <div class="flex items-center">
                ...
                <a :class="step === 5 ? 'text-primary hover:text-primary-darker-1' : 'text-gray-500 hover:text-gray-700'"
        ...
        <div class="md:px-32" v-if="step === 1">
              ...
              <div class="mt-6 grid grid-cols-1 gap-3 md:gap-6">
                ...
                <div class="px-4">
                  ...
                  ></vue-tel-input>
                    ...
                    <small v-if="errors && errors.phone_number" class="text-red-500">{{errors.errors.phone_number[0]}}</small>
                  ...
                  <fieldset class="mt-6 px-4" v-if="existingStudents.length">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 mt-4 space-y-2">
                      <div class="flex items-center" v-for="s in existingStudents">
                        <input type="radio" name="student" :value="s" v-model="reschedule.student" class="focus:ring-primary h-4 w-4 text-primary border-gray-300" required>
          ...
          <div class="text-center" v-if="existingStudents.length">
            <fieldset class="mt-6 px-4">
              <p class="text-sm text-gray-500 text-left flex">Select verification method: <span class="text-danger font-bold text-xl">*</span> </p>
              <div class="grid grid-cols-1 md:grid-cols-3 mt-4">
                <div class="flex items-center">
                  <input type="radio" name="verification" value="code" v-model="reschedule.type" class="focus:ring-primary h-4 w-4 text-primary border-gray-300" required>
                  <label class="ml-3 block text-sm text-gray-700 capitalize">Get Verification Code</label>
                </div>
                <div class="flex items-center">
                  <input type="radio" name="verification" value="password" v-model="reschedule.type" class="focus:ring-primary h-4 w-4 text-primary border-gray-300" required>
                  <label class="ml-3 block text-sm text-gray-700 capitalize">Enter Students Password</label>
                </div>
              </div>
            </fieldset>
          </div>
          <div class="pt-5 text-center px-3">
            <transition name="fade">
              <button @click="onPhoneNumberEnter" v-if="!existingStudents.length" :disabled="!this.phoneObject.valid" type="button" class="w-full md:w-52 disabled:opacity-50 disabled:cursor-not-allowed shadow-primary space-x-2 inline-flex items-center justify-center py-2 text-sm font-medium border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none ">
                <span>Fetch Students</span>
                <!-- <Loader v-if="buttonLoader" variants="animate-spin" /> -->
              </button>
              <button v-else @click="submitStepOne" type="button" :disabled="buttonLoader || !reschedule.student.id || !reschedule.type" class="w-full md:w-52 disabled:opacity-50 disabled:cursor-not-allowed shadow-primary space-x-2 inline-flex items-center justify-center py-2 text-sm font-medium border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none ">
                <span>Next</span>
                <!-- <Loader v-if="buttonLoader" variants="animate-spin" /> -->
              </button>
            </transition>
      ...
      <transition :name="slide_animation">
        <form class="md:px-32 md:space-y-5 px-4" v-if="step === 2" @submit.prevent="studentVerification">
          <div v-if="reschedule.type === 'password'">
            <label class="block text-sm text-gray-700 flex">Password <span class="text-danger font-bold text-xl">*</span> </label>
            <div class="mt-1">
              <input
              ...
              <div class="flex items-center justify-start mt-2">
                ...
                <div class="text-primary cursor-pointer text-sm " @click="resetPassword">Forgot password? Click to reset!</div>
              </div>
            </div>
          </div>
          <div v-if="reschedule.type === 'code'">
            <label class="block text-sm text-gray-700 flex pb-2 flex">Enter Verification Code: <span class="text-danger font-bold text-xl">*</span> </label>
            <div class="mt-1">
              <input
                v-model="reschedule.pass_code"
                type="number"
                ...
                class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full"
              >
              <div @click="reschedule.type = 'password'" class="text-primary cursor-pointer text-sm mt-1">Did not receive SMS, click to enter student password!</div>
            </div>
          </div>
          <div class="py-2">
            <button type="submit" :disabled="buttonLoader" class="w-full disabled:opacity-50 disabled:cursor-not-allowed shadow-primary space-x-2 inline-flex items-center justify-center py-2 text-sm font-medium border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none ">
              <span>Verify</span>
              <!-- <Loader v-if="buttonLoader" variants="animate-spin" /> -->
            </button>
          </div>
      ...
      <div class="text-center" v-if="step >= 3 && step < 5">
        <div v-if="reschedule.selectedClassToReschedule">
          ...
          <span class="capitalize">{{ reschedule.selectedClassToReschedule.name }}</span>, {{reschedule.selectedClassToReschedule.starts_at}}, ({{filters.format_time_zone(timezone)}})
          <span v-if="reschedule.selectedClassToReschedule.teacher.length">, Teacher: {{ reschedule.selectedClassToReschedule.teacher[0].full_name }}</span>
        </div>
      </div>
      ...
      <transition :name="slide_animation">
        <div class="md:px-32 md:space-y-5" v-if="step === 3">
          <div>
            <h2 class="text-lg leading-6 font-medium text-center" :class="reschedule.selectedClassToReschedule ? 'text-green-500' : 'text-red-500'">

              <span v-if="available_time_slots.length">
                <span class="text-left">Classes available to reschedule please click on one to select:</span>
              </span>
              <span class="text-lg leading-6 font-medium text-danger" v-if="!tasks_classes.length && step === 3">
            ...
            <fieldset class="mt-2 space-y-2 px-4">
              <div class="divide-y divide-gray-200" v-for="task_class in tasks_classes">
                ...
                <div class="relative flex flex-col md:flex-row items-start gap-2 py-4 border p-2 rounded-2xl items-center cursor-pointer" @click="rescheduleNow(task_class)" :class="reschedule.selectedClassToReschedule.id === task_class.id ? 'border-primary border-2 bg-indigo-100' : ''">
                  <div class="min-w-0 flex-1 text-sm">
                    <label class="font-medium text-gray-700 capitalize"><span class="text-primary">Class:</span> {{ task_class.name }}</label>
                    <p id="account-mastercard-description" class="text-gray-500"><span class="text-primary">Starts At:</span> {{ task_class.starts_at }} ({{filters.format_time_zone(timezone)}})</p>
                    <p class="text-gray-500 capitalize" v-if="task_class.teacher.length"><span class="text-primary">Teacher:</span> {{ task_class.teacher[0].full_name }}</p>
                  </div>
                  ...
                  <div class="w-full md:w-1/2 flex items-center gap-2" v-if="!task_class.parent_id && task_class.is_absent_rescheduled_after_class_starts === 0">
                    <button v-if="task_class.students[0].pivot.rescheduled !== 2"  @click.stop="cancelClass(task_class)" type="button" class="w-full md:w-1/2 shadow-danger py-2 text-sm font-medium border border-danger rounded-full text-white bg-danger hover:bg-danger-darker-1 focus:outline-none ">
                        Cancel
                      </button>
                      <button @click.stop="testNew(task_class)" type="button" class="w-full md:w-1/2 shadow-primary py-2 text-sm font-medium border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none ">
                        Reschedule
                      </button>
                      <div class=" flex items-center h-5">
                    </div>
                  </div>
                  <div class="flex flex-col md:flex-row gap-2 text-warning" v-else-if="!task_class.parent_id && task_class.is_absent_rescheduled_after_class_starts === 1">
      ...
      <transition :name="slide_animation">
        <form v-if="step === 4" @submit.prevent="next" class="divide-y divide-black-lighter-4">
          <div class="space-y-8 pb-20">
            <div class="py-2 px-5">
              <div class="">
                <label class="block text-sm text-gray-700 flex">Teachers <span class="text-danger font-bold text-xl">*</span> </label>
                <div class="mt-1">
                  <select v-model="reschedule.showAllTeacher" @change="handleChangeInTeacher()" class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                    <option selected disabled value="">Select Teacher</option>
                    <option selected :value="true">All Teachers</option>
                    <option selected :value="false">Current Teacher</option>
                  </select>
                  <small v-if="errors && errors.location_id" class="text-red-500">{{errors.location_id[0]}}</small>
                </div>
              </div>
              <div class="mt-6 grid grid-cols-1 md:grid-cols-2 md:gap-2">
                ...
                <div class="space-y-1 max-h-96 overflow-y-auto px-0 md:px-10">
                  <div class="text-primary flex flex-col items-center justify-center md:flex-row" v-if="reschedule.selected_date">
                    <div> {{ filters.day_date(reschedule.selected_date_raw) }}</div>
                  ...
                  <span>
                    <div class="flex items-center justify-center px-10 py-10" v-if="Object.keys(available_time_slots).length === 0 && !loader">
                      <p class="text-green-500 text-center">Please select another date in calendar to load available time slots</p>
                    </div>
                  ...
                  <div class="grid grid-cols-3 gap-1 md:gap-4 text-center text-primary" v-if="available_time_slots && Object.keys(available_time_slots).length > 0">
                    <span class="p-1 md:p-3 md:mx-2">Time</span>
                    <span class="col-span-2 p-1 md:p-3 md:mx-2">Teacher</span>
                  </div>

                  <div class="grid grid-cols-3 cursor-pointer gap-1 md:gap-4" v-for="(slot) in available_time_slots" :key="slot.slot" @click="selectClassHandler(slot)">
                    <button type="button" class="p-1 md:p-3 md:mx-2 rounded-xl text-center"
                            :class="reschedule.slot.id === slot.id && reschedule.slot.teacher_id === slot.teacher_id && reschedule.slot.time_int === slot.time_int ? 'bg-primary text-white w-full' : 'text-info bg-info-lighter-4 w-full'"
                    >{{ filters.timeOnly(slot.starts_at_local_time)}}</button>
          ...
          <div class="pt-10 px-4">
            <div class="flex justify-center">
              <button type="submit" :disabled="Object.keys(reschedule.slot).length < 2" class="disabled:opacity-50 disabled:cursor-not-allowed shadow-primary w-full md:w-max px-20 py-2 text-sm font-medium border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none ">
                <span>Next</span>
                <!-- <Loader v-if="buttonLoader" variants="animate-spin" /> -->
              </button>
            </div>
          ...
          <div class="space-y-8 pb-20">
            <div class="py-2 px-5">
              <div class="text-center rounded-lg py-10">
                {{!this.reschedule.cancel ? 'Rescheduling a Make-Up Class Confirmation' : 'Cancel & Reschedule A Make-Up class Later'}}
                <p>
              ...
              <form @submit.prevent="rescheduleClass">
                <div class="space-y-20" v-if="!reschedule.cancel">
                  <div class="md:pl-52 space-y-2">
                    <p><span class="font-bold"><span class="text-primary">Time:</span></span> {{reschedule.slot.starts_at_local_time}} ({{filters.format_time_zone(timezone)}})  </p>
                    <p><span class="font-bold"><span class="text-primary">Teacher:</span></span> <span class="capitalize"> {{reschedule.slot.teacher_name}} </span> </p>
                    <p><span class="font-bold"><span class="text-primary">Student Name:</span></span> <span class="capitalize"> {{reschedule.student.full_name}}  </span> </p>
                    <p><span class="font-bold"><span class="text-primary">Email:</span></span> {{reschedule.student.email}}  </p>
                    <p><span class="font-bold"><span class="text-primary">Phone Number:</span></span> {{ reschedule.student.phone_number }}</p>
                    <p><span class="font-bold"><span class="text-primary">Location:</span></span>  {{reschedule.selectedClassToReschedule.location.location_name}}</p>
                  </div>
                  <div class="text-center space-y-2">
                    <button @click="back" type="button" class="shadow-primary w-full md:w-max px-20 py-2 text-sm font-medium border border-primary rounded-full text-primary hover:bg-primary hover:text-white focus:outline-none ">
                      Cancel
                    </button>
                    <button :disabled="buttonLoader" type="submit" class="shadow-primary w-full md:w-max px-20 py-2 text-sm font-medium border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none ">
                ...
                <div class="text-center">
                  <button v-if="reschedule.cancel" :disabled="buttonLoader" type="submit" class="shadow-primary space-x-2 inline-flex items-center px-20 py-2 text-sm font-medium border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none ">
      ...
      <transition :name="slide_animation">
        <section class="text-center py-16" v-if="step === 6">
          ...
          <p v-if="!reschedule.cancel">You successfully rescheduled a make-up class, as per details below:</p>
          <p v-else>You successfully cancelled your class, you can visit this form to choose another schedule later:</p>
