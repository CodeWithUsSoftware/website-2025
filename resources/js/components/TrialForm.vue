                    ...
                    <ol role="list" class="bg-white rounded-md shadow px-6 flex space-x-4">
                        <li class="flex">
                            <div class="flex items-center">
                                <a href="#freeform" @click="step = 1"
                                    ...
                                    <div v-else>
                                        <a @click="step = 1" href="#freeform" class="ml-4 text-sm" :class="step === 1
                        ...
                        <li class="flex" v-if="step > 1">
                            <div class="flex items-center">
                                ...
                                <a @click="step = 2" href="#freeform" class="ml-4 text-sm" :class="step === 2
                                        ? 'text-primary hover:text-primary-darker-1'
                                        : 'text-gray-500 hover:text-gray-700'
                                    ">Student Info</a>
                            </div>
                        ...
                        ">
                            <div class="flex items-center">
                                ...
                                <a href="#freeform" class="ml-4 text-sm" :class="step === 4
                                        ? 'text-primary hover:text-primary-darker-1'
                                        : 'text-gray-500 hover:text-gray-700'
                                    ">Payment</a>
                            </div>
            ...
            <transition :name="slide_animation">
                <form v-if="step === 1" @submit.prevent="next" class="divide-y divide-black-lighter-4">
                    <div class="space-y-8 pb-20">
                        <div class="pt-2">
                            <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-6">
                                <div class="">
                                    <label class="block text-sm text-gray-700 flex">Location<span
                                            class="text-danger font-bold text-xl">*</span></label>
                                    <div class="mt-1">
                                        <select v-model="registration.location_id" @change="
                                            ...
                                            <option v-for="location in locations.locations" :key="location.id"
                                                :value="location.id">
                                                {{ location.location_name }}
                                            </option>
                                        </select>
                                        <small class="text-danger" v-if="error.errors.location_id">{{
                                ...
                                <div class="">
                                    <label class="block text-sm text-gray-700 flex">Topic
                                        <span class="text-danger font-bold text-xl">*</span></label>
                                    <div class="mt-1">
                                        <select v-model="registration.topic_id" @change="
                                        ...
                                        <small class="text-danger" v-if="error.errors.topic_id">{{ error.errors.topic_id
                                            }}</small>
                                ...
                                <div class="">
                                    <label class="block text-sm text-gray-700 flex">Class Type
                                        <span class="text-danger font-bold text-xl">*</span></label>
                                    <div class="mt-1">
                                        <select v-model="registration.class_type" @change="reloadData"
                                            class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                                            <option selected disabled value="">
                                                Select Class Type
                                            </option>
                                        ...
                                        <small class="text-danger" v-if="error.errors.class_type">{{
                            ...
                            <div class="mt-6 grid grid-cols-1 md:grid-cols-2 md:gap-2">
                                <vue-cal style="height: 350px" @view-change="ready" @ready="ready" :time="false"
                                    ref="vueCal" active-view="month" :disable-views="[
                                        ...
                                        'week',
                                        'day',
                                    ]" hide-view-selector xsmall :selected-date="registration.selected_date_raw
                    ...
                    >Teacher</button>
                                    ...
                                    <div class="grid grid-cols-3 cursor-pointer gap-1 md:gap-4" v-for="(
slot, index
                                        ) in available_time_slots" :key="index" v-if="registration.selected_date"
                                        @click="selectClassHandler(slot)">
                                        <button type="button" class="p-1 md:p-3 md:mx-2 rounded-xl text-center" :class="registration.slot.slot ===
                                                slot.slot &&
                                        ...
                                        <button type="button" class="col-span-2 p-1 md:p-3 md:mx-2 rounded-xl" :class="registration.slot.slot ===
                                                slot.slot &&
                                                registration.slot.teacher.id ===
                                                slot.teacher.id
                                                ? 'bg-primary text-white w-full'
                    ...
                    <div class="pt-10">
                        <div class="flex justify-center">
                            <button type="submit" :disabled="!registration.slot.slot"
                    ...
                    <div class="space-y-8">
                        <div class="pt-2">
                            <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6">
                                <div class="">
                                    ...
                                    <div class="mt-1">
                                        <vue-tel-input v-bind="bindProps" :preferredCountries="preferredCountries
                                            " :autoFormat="false" @input="phoneInput" @blur="onPhoneNumberEnter"
                                            @country-changed="
                                                onPhoneNumberEnter
                                            " v-model="phone" :autoDefaultCountry="true" :defaultCountry="countries.countries
                                                    .character_code
                                ...
                                <div class="">
                                    <label class="block text-sm text-gray-700 flex">Email
                                        <span class="text-danger font-bold text-xl">*</span></label>
                                    <div class="mt-1">
                                        <input required type="email" :placeholder="free_session_email"
                    ...
                    >
                                    ...
                                    <fieldset class="mt-6" v-if="existingStudents.length">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 mt-4 space-y-2">
                                            <div class="flex items-center" v-for="student in existingStudents">
                                                <input type="radio" name="student" :value="student" v-model="registration.student
                                                    " class="focus:ring-primary h-4 w-4 text-primary border-gray-300"
                                                    required />
                                                <label class="ml-3 block text-sm text-gray-700 capitalize">{{
                                                    student.full_name
                                                }}</label>
                                            </div>
                                        </div>
                                ...
                                <div class="md:col-span-2 text-info" v-if="existingStudents.length">
                                    <div class="flex items-center justify-end space-x-3">
                                        ...
                                        <p class="text-info cursor-pointer" @click="addNewStudentHandler">
                                            Add a New Student
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="" v-if="registration.addNewStudent">
                                    <label class="block text-sm text-gray-700 flex">Student Name
                                        <span class="text-danger font-bold text-xl">*</span></label>
                                    <div class="mt-1">
                                        <input type="text" v-model="registration.student.full_name
                                            " required
                                            class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full" />
                                    </div>
                                </div>
                                ...
                                <div class="" v-if="registration.addNewStudent">
                                    <label class="block text-sm text-gray-700 flex">Age
                                        <span class="text-danger font-bold text-xl">*</span></label>
                                    <div class="mt-1">
                                ...
                                <div class="md:col-span-2 pt-5" v-if="!registration.student.id">
                                    <label class="block text-sm text-gray-700 flex">How did you hear about us?
                                        <span class="text-danger font-bold text-xl">*</span></label>
                                    <div class="mt-1">
                    ...
                    <div class="space-y-10">
                        <form @reset="back">
                            ...
                            <div class="md:pl-60 space-y-2">
                                <p>
                                    <span class="font-bold"><span class="text-primary">Class:</span></span>
                                    {{
                                        filters.day_date(
                                            registration.selected_date_raw
                                        )
                                ...
                                <p>
                                    <span class="font-bold"><span class="text-primary">Amount:</span></span>
                                    ${{
                            ...
                            <div class="flex items-center justify-center text-center pt-5">
                                ...
                                <label class="ml-3 block text-sm text-gray-400 mt-1">
                                    I agree to the
                                    <a :href="drive_link" class="text-primary hover:text-primary-darker-1"
                                        target="_blank">terms of
                                        service, billing and privacy
                                        policy</a>
                                    of Code With Us
                                </label>
                            </div>
                            ...
                            <div class="text-center space-y-3 md:space-y-0">
                                <button :disabled="termsAndConditions ? false : true
                                    " type="button"
                                    class="disabled:opacity-50 disabled:cursor-not-allowed shadow-primary w-full md:w-max px-20 py-2 text-sm font-medium border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none"
                                    @click="
                                        if (
            ...
            <transition :name="slide_animation">
                <section class="py-10" v-show="step === 4">
                    <form>
                        <div class="mx-auto justify-center space-y-16 divide-y"
                            :class="step === 4 ? 'max-w-5xl' : 'max-w-2xl'">
                ...
                <section class="text-center py-10" v-if="step === 5">
                    ...
                    <div class="pt-10 space-y-2">
                        ...
                        <p class="pt-10 text-primary font-extrabold">
                            <a href="https://www.codewithus.com/school-program#freeform">Please click here to learn how
                                to access
                                your
                                trial class</a>
                        </p>
...
import useTrialFormMixin from '../mixins/useTrialFormMixin.js'
...
export default {
    ...
    mixins: [partner_code_cookie, recaptcha, useTrialFormMixin],
    props: {
        phone_number: {
            default: false,
        },
