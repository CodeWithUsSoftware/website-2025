<template>
  <div class="px-3 md:px-0">
    <div id="freeform" class="space-y-8 bg-white shadow-card rounded-3xl md:p-10 px-10 py-10 overflow-x-hidden">
          ...
          <ol role="list" class="bg-white rounded-md shadow px-6 flex space-x-4 ">
            <li class="flex">
              <div class="flex items-center">
                <a href="#freeform" @click="step = 1" class="text-gray-400 hover:text-gray-500 cursor-pointer" :class="step === 1 ? ' py-2 ' : ''">
                  <div v-if="step === 1">
                    <img
                      :class="step === 1 ? 'bg-primary hover:bg-primary-darker-1 w-20 h-20' : 'bg-gray-500 hover:bg-gray-700'"
                      width="24" height="24" src="/images/trial_form/trial_logo.webp" alt="">
                    <span class="sr-only">Home</span>
                  </div>
                  <div v-else>
                    <a @click="step = 1" href="#freeform" class="ml-4 text-sm"
            ...
            <li class="flex" v-if="step > 1">
              <div class="flex items-center">
                ...
                <a @click="step = 2" href="#freeform" class="ml-4 text-sm"
            ...
            <li class="flex" v-if="step > 2">
              <div class="flex items-center">
                ...
                <a
                  ...
                  href="#freeform" class="ml-4 text-sm" aria-current="page">Order Confirmation</a>
              </div>
            </li>

            <li class="flex" v-if="registration.class_type === 'private' && step > 3">
              <div class="flex items-center">
                <svg class="flex-shrink-0 w-6 h-full text-gray-200" viewBox="0 0 24 44" preserveAspectRatio="none" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                  <path d="M.293 0l22 22-22 22h1.414l22-22-22-22H.293z" />
                </svg>
                <a href="#freeform" class="ml-4 text-sm"
      ...
      <div v-if="step < 5" class="sm:hidden bg-white rounded-md shadow px-4 py-3">
        <div class="flex flex-col items-center justify-center space-y-2">
          ...
          <span class="text-sm font-medium text-gray-700 text-center">
            ...
            <template v-else-if="step === 4 && registration.class_type === 'private'">Payment</template>
          </span>
      ...
      <div class="md:px-36">
        <h3 class="text-center py-8" v-if="step < 4">Schedule a Trial Class!</h3>
        <p class="text-center -mt-5" v-if="step < 4">{{registration.location ? registration.location.address : ''}}</p>
        <p class="text-center" v-if="step === 1">
          Please choose your class preferences below, then click on the days you
          are interested in on the calendar to see class time and availability.
        </p>
      </div>

      <transition :name="slide_animation">
        <form v-if="step === 1" @submit.prevent="next" class="divide-y divide-black-lighter-4">
          <div class="space-y-8 pb-20">
            <div class="pt-2">
              <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-6">
                <div class="">
                  <label class="block text-sm text-gray-700 flex">Location<span class="text-danger font-bold text-xl">*</span></label>
                  <div class="mt-1">
                    <select v-model="registration.location_id" @change="handleLocationSelection(registration.location_id)" class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                      <option selected disabled value="">Select Location</option>
                      <option v-for="location in locations.locations" :key="location.id" :value="location.id">{{location.location_name}}</option>
                    </select>
                    <small class="text-danger" v-if="error.errors.location_id">{{error.errors.location_id}}</small>
                    <!-- <small v-if="errors && errors.location_id" class="text-red-500">{{errors.location_id[0]}}</small> -->
                  </div>
                </div>

                <div class="">
                  <label class="block text-sm text-gray-700 flex">Topic  <span class="text-danger font-bold text-xl">*</span></label>
                  <div class="mt-1">
                    <select v-model="registration.topic_id" @change="handleTopicSelection(registration.topic_id)" class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                    ...
                    <small class="text-danger" v-if="error.errors.topic_id">{{error.errors.topic_id}}</small>
                    <!-- <small v-if="errors && errors.topic_id" class="text-red-500">{{errors.topic_id[0]}}</small> -->
                  </div>
                </div>

                <div class="">
                  <label class="block text-sm text-gray-700 flex">Class Type  <span class="text-danger font-bold text-xl">*</span></label>
                  <div class="mt-1">
                    <select v-model="registration.class_type" @change="reloadData" class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                    ...
                    <small class="text-danger" v-if="error.errors.class_type">{{error.errors.class_type}}</small>
                    <!-- <small v-if="errors && errors.class_type" class="text-red-500">{{errors.class_type[0]}}</small> -->
                  </div>
                </div>
              </div>
              <div class="mt-6 grid grid-cols-1 md:grid-cols-2 md:gap-2">
                ...
                <div class="space-y-1 max-h-96 overflow-y-auto p-0 md:px-10">
                  ...
                  <!-- <div class="grid grid-cols-3 cursor-pointer gap-1 md:gap-4" v-if="available_time_slots">
                    <button type="button" class="p-1 md:mx-2 text-primary text-center w-full"
                    >Time</button>
                    <button type="button" class="p-1 md:mx-2 text-primary text-center w-full"
                    >Teacher</button>
                  </div> -->
                  ...
                  <div class="grid grid-cols-3 cursor-pointer gap-1 md:gap-4" v-for="(slot, index) in available_time_slots" :key="index" v-if="registration.selected_date" @click="selectClassHandler(slot)">
                    ...
                    >{{slot.slot_display}}</button>
                    <button type="button" class="col-span-2 p-1 md:p-3 md:mx-2 rounded-xl"
                            :class="registration.slot.slot === slot.slot && registration.slot.teacher.id === slot.teacher.id ? 'bg-primary text-white w-full' : 'text-info bg-info-lighter-4  w-full'"
                    >{{slot.teacher.full_name}}</button>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="pt-10">
            <div class="flex justify-center">
              <button type="submit" :disabled="!registration.slot.slot" class="disabled:opacity-50 disabled:cursor-not-allowed shadow-primary w-full md:w-max px-20 py-2 text-sm font-medium border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none ">
                <span>Next</span>
            ...
            <div class="flex md:flex-col items-center justify-center space-x-3 py-3 pt-10 text-sm">
              ...
              <p class="underline">
                ...
                <!-- <nuxt-link :to="'/contact'">Send a custom request to the team</nuxt-link> -->
              </p>
            </div>
          </div>
        </form>
      ...
      <transition :name="slide_animation">
        <form class="md:px-32" v-if="step === 2" @submit.prevent="next">
          <div class="space-y-8">
            <div class="pt-2">
              <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6">
                ...
                <div class="">
                  ...
                  <div class="mt-1">
                    <vue-tel-input
                      ...
                      required
                    ></vue-tel-input>
                    <small v-if="errors && errors.phone_number" class="text-red-500">{{errors.phone_number[0]}}</small>
                  </div>
                </div>
                ...
                <div class="">
                  ...
                  <div class="mt-1">
                    <input required type="email" :placeholder="free_session_email" v-model="registration.parent.email" class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                  </div>
                </div>

                <!-- <div class="md:col-span-2 pt-5">
                  <label class="block text-sm text-gray-700 flex">Parent Name  <span class="text-danger font-bold text-xl">*</span></label>
                  <div class="mt-1">
                    <input
                ...
                <div class="md:col-span-2" >
                  <fieldset class="mt-6" v-if="existingStudents.length">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 mt-4 space-y-2">
                      <div class="flex items-center" v-for="student in existingStudents">
                ...
                <div class="" v-if="registration.addNewStudent">
                  ...
                  <div class="mt-1">
                    <input type="text" v-model="registration.student.full_name" required class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                  </div>
                </div>
                ...
                <div class="" v-if="registration.addNewStudent">
                  ...
                  <div class="mt-1">
                    <input type="number" v-model="registration.student.age" required class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                  </div>
                </div>
          ...
          <div class="pt-5">
            <div class="flex justify-center">
              <button type="submit" :disabled="buttonLoader || !registration.student.full_name" class="disabled:opacity-50 disabled:cursor-not-allowed shadow-primary w-full md:w-max px-20 py-2 text-sm font-medium border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none ">
                <span>Next</span>
                <!-- <Loader v-if="buttonLoader" variants="animate-spin" /> -->
              </button>
            </div>
      ...
      <transition :name="slide_animation">
        <section v-if="step === 3">
          <div class="space-y-10">
            <form @reset="back">
              <h5 class="text-center">Are you sure you want to book this class?</h5>
              <div class="md:pl-60 space-y-2">
                <p><span class="font-bold"><span class="text-primary">Class:</span></span> {{filters.day_date(registration.selected_date_raw)}} </p>
                <p><span class="font-bold"><span class="text-primary">Time:</span></span> {{ registration.slot.slot_display }} ({{filters.format_time_zone(timezone)}}) </p>
                ...
                <p><span class="font-bold"><span class="text-primary">Amount:</span></span> ${{ registration.class_type !== 'group' ? private_session_fee : 0}} </p>
              </div>


              <div class="flex items-center justify-center text-center pt-5">
                <input required type="checkbox" name="student" v-model="termsAndConditions" class="focus:ring-primary h-4 w-4 text-primary border-gray-300 bg-gray-100 rounded">
              ...
              <div class="text-center space-y-3 md:space-y-0">
                <button :disabled="termsAndConditions ? false : true" type="button" class="disabled:opacity-50 disabled:cursor-not-allowed shadow-primary w-full md:w-max px-20 py-2 text-sm font-medium border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none " @click="if(this.registration.class_type != 'group'){redirectToCheckout();} else {next(); }">
                  <span>Book A Class</span>
                  <!-- <Loader v-if="buttonLoader" variants="animate-spin" /> -->
                </button>
                <button type="reset" class="shadow-primary w-full md:w-max px-20 py-2 text-sm font-medium border border-primary rounded-full text-primary hover:bg-primary hover:text-white focus:outline-none ">
                  Cancel
                </button>
              </div>
            </form>
          ...
          <form>
            <div class="mx-auto justify-center space-y-16 divide-y" :class="step === 4 ? 'max-w-5xl' : 'max-w-2xl'">
              <!-- <div class="flex justify-between">
                <h6 class="font-bold uppercase">Trial Class</h6>
                <h6>${{private_session_fee}}</h6>
              </div>
              <div class="pt-10">
              ...
              <!-- <div class="flex justify-between pt-2">
                ...
                <h6 class="font-bold text-primary">${{private_session_fee}}</h6>
              </div> -->
            </div>
      ...
      <transition :name="slide_animation">
        <section class="text-center py-10" v-if="step === 5">
          <h5 class="text-primary"><span class="text-primary">Thank you!</span></h5>
          <p>You successfully booked a class, as per details below:</p>
          <div class="pt-10 space-y-2">
            ...
            <p><span class="font-bold"><span class="text-primary">Phone Number:</span></span> {{phoneObject.countryCallingCode}} {{registration.parent.phone_number}} </p>
            <p><span class="font-bold"><span class="text-primary">Amount:</span></span> ${{ registration.class_type !== 'group' ? private_session_fee : 0}} </p>
            <p class="pt-10 text-primary font-extrabold"><a href="https://www.codewithus.com/school-program#freeform">Please click here to learn how to access your trial class</a></p>
          </div>
        </section>
      </transition>
