<template>
 <div class="px-3 md:px-0">
    <div id="pricing_div" class="space-y-8 bg-white shadow-card rounded-3xl p-10 md:px-10 overflow-x-hidden">
            ...
            <ol role="list" class="bg-white rounded-md shadow px-6 flex space-x-4">
              <li class="flex p-3" v-if="step >= 0">
                <div class="flex items-center">
                  <a @click="step = 0" href="#pricing_div" class="ml-4 text-sm"
                     :class="step === 0 ? 'text-primary hover:text-primary-darker-1' : 'text-gray-500 hover:text-gray-700'"
                  >Class Options</a>
                </div>
              </li>
              ...
              <li class="flex" v-if="step > 1">
                <div class="flex items-center">
                  ...
                  <a @click="step = 2" href="#pricing_div" class="ml-4 text-sm"
              ...
              <li class="flex" v-if="step > 2">
                <div class="flex items-center">
                  ...
                  <a @click="step = 3"
                     ...
                     href="#pricing_div" class="ml-4 text-sm" aria-current="page">Order Confirmation</a>
                </div>
              </li>
              ...
              <li class="flex" v-if="step > 3">
                <div class="flex items-center">
                  ...
                  <a href="#pricing_div" class="ml-4 text-sm"
        ...
        <div v-if="step < 5" class="sm:hidden bg-white rounded-md shadow px-4 py-3">
            <div class="flex flex-col items-center justify-center space-y-2">
              ...
              <span class="text-sm font-medium text-gray-700 text-center">
                ...
                <template v-else-if="step === 1">Class Schedule</template>
        ...
        <transition :name="slide_animation">
          <section v-if="step === 0" class="py-10 md:py-20">
            ...
            <form @submit.prevent="step++">
                <div class="grid grid-cols-1 md:grid-cols-12 sm:grid-cols-12 gap-12">
                    <div class="col-span-6">
                      <div>
                        <div class="text-center mb-5">
                          <h6 class="text-xl">Select Your
                            ...
                            Plan Preferences </h6>
                          <h6 v-if="registration.billing_cycle === 1">No Contracts. Month to Month. Cancel Anytime.</h6>
                          <h6 v-else>No Contracts. Cancel Anytime Before Renewal.</h6>
                        </div>
                        <div class="flex justify-center mt-5 space-y-3">
                          <div class="flex relative">
                            <div
                              class="bg-white hidden md:inline-block py-2 min-h-full w-20 md:w-16 transform rotate-45 translate-y-2 md:translate-y-0 -mr-5 z-20"></div>
                            <div class="bg-yellow-400 inline-block p-4 px-8 md:px-14 z-10 text-center">
                              <h6 class="font-extrabold">Limited Time :
                                <span v-if="registration.billing_cycle === 1">{{'$'+ discounted_price}}</span>
                                <span v-if="registration.billing_cycle === 3">{{'$'+ discounted_price}}</span>
                                <span v-if="registration.billing_cycle === 6">{{'$'+ discounted_price}}</span>
                                off on Classes</h6>
                            </div>
                            <div
                        ...
                        <div class="px-4">
                          <div class="pt-5">
                            <h6 class="font-bold text-sm flex">Choose the location for the coding classes <span class="text-danger font-bold text-xl">*</span></h6>
                            <div class="mt-1">
                              <select required v-model="registration.location_id" @change="handleLocationSelection(registration.location_id)" :class="errors && errors.location_id ? 'border-red-500' : ''" class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                                <option selected disabled value="">Select Location</option>
                                <option v-for="location in locations.locations" :key="location.id" :value="location.id">{{location.location_name}}</option>
                              </select>
                              <small v-if="errors && errors.location_id" class="text-red-500">{{errors.location_id}}</small>
                            </div>
                          </div>

                          <div class="space-y-3 mt-3 mb-3">
                            <h6 class="font-bold text-sm flex">Choose the coding topic for the classes<span class="text-danger font-bold text-xl">*</span></h6>
                              <div class="mt-1">
                                <select required v-model="registration.topic_id" @change="handleTopicSelection(registration.topic_id)" :class="errors && errors.topic_id ? 'border-red-500' : ''" class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                          ...
                          <div class="space-y-3 mt-3 mb-3">
                            <h6 class="font-bold text-sm flex">Choose the timeslot Duration<span class="text-danger font-bold text-xl">*</span></h6>
                                <div class="mt-1">
                                  <select required v-model="registration.timeslot_duration" @change="handleTopicSelection(registration.topic_id)" class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                                       <option selected  disabled value="">Select Timeslot Duration</option>
                          ...
                          <div class="space-y-3 mb-4">
                            <h6 class="font-bold text-sm">Choose the class size</h6>
                            <div class="flex px-2 space-x-2">
                              <input required value="Small Group" :checked="registration.class_type === 'Small Group'" type="radio" v-model="registration.class_type" @change="handleClassTypeChange" :class="errors && errors.class_type ? 'border-red-500' : ''"  class="shadow-sm focus:ring-camps focus:border-camps block w-5 h-5  sm:text-sm border-gray-300 bg-white ">
                              <label class="text-black-lighter-2 text-sm">Small Group(3:1)</label>
                            </div>
                            <div class="flex px-2 space-x-2">
                              <input required value="Private" :checked="registration.class_type === 'Private'" type="radio" v-model="registration.class_type" @change="handleClassTypeChange" :class="errors && errors.class_type ? 'border-red-500' : ''"  class="shadow-sm focus:ring-camps focus:border-camps block w-5 h-5  sm:text-sm border-gray-300 bg-white ">
                              <label class="text-black-lighter-2 text-sm">Private (1:1)</label>
                            </div>
                            <small v-if="errors && errors.class_type" class="text-red-500">{{errors.class_type}}</small>
                          </div>

                          <div class="space-y-3 mb-4">
                            <h6 class="font-bold text-sm">Choose the class frequency</h6>
                            <div class="flex px-2 space-x-2">
                              <input required type="radio" :value="parseInt(1)" :checked="registration.class_per_week === 1" v-model="registration.class_per_week" @click="handleClassPerWeekChange" :class="errors && errors.class_per_week ? 'border-red-500' : ''" class="shadow-sm focus:ring-camps focus:border-camps block w-5 h-5  sm:text-sm border-gray-300 bg-white ">
                              <label class="text-black-lighter-2 text-sm">One per week</label>
                            </div>
                            <div class="flex px-2 space-x-2">
                              <input required type="radio" :value="parseInt(2)" :checked="registration.class_per_week === 2" v-model="registration.class_per_week" @click="handleClassPerWeekChange" :class="errors && errors.class_per_week ? 'border-red-500' : ''" class="shadow-sm focus:ring-camps focus:border-camps block w-5 h-5  sm:text-sm border-gray-300 bg-white ">
                              <label class="text-black-lighter-2 text-sm">Two per week</label>
                            </div>
                    ...
                    <div class="col-span-6">
                      <div class=" bg-gradient-to-t p-[10px] from-primary to-success rounded-lg h-full">
                        <div class="p-4 bg-white rounded-lg h-full">

                          <div class="h-full flex flex-col  overflow-hidden lg:rounded-none lg:rounded-l-2xl">
                            <div class="flex-1 flex flex-col">
                              <div class="text-center">
                                ...
                                <div class="flex flex-col justify-center md:flex-row items-center md:space-x-6 mt-1">
                                  <h3 class="text-2xl relative">
                                    {{'$'+ Math.ceil(total_amount / .8)}}
                                    <span class="absolute right-0 border border-danger left-0 top-8"></span>
                                  </h3>
                                  <h1 class="md:text-5xl font-extrabold text-primary">{{'$'+ total_amount}}</h1>
                                </div>
                                <br>
                                <span class="text-sm" v-if="registration.billing_cycle === 1">({{'$'+ discounted_price}} Discount is already included in price above)</span>
                                <span class="text-sm" v-if="registration.billing_cycle === 3">({{'$'+ discounted_price}} Discount is already included in price above)</span>
                                <span class="text-sm" v-if="registration.billing_cycle === 6">({{'$'+ discounted_price}} Discount is already included in price above)</span>
                              </div>
                              <div class="text-center">
                                <h5 class="underline">Your Subscription Details</h5>
                              </div>
                              <div class="flex-1 flex flex-col justify-between p-6 bg-white sm:p-10 lg:p-6 xl:p-10">
                                <ul role="list" class="space-y-4">
                                  <li class="flex items-start">
                                    <p class="ml-3 text-base font-medium text-gray-500">
                                      <span class="font-bold text-primary">Location: </span>
                                      <span v-if="registration.location_id">{{ registration.location.location_name }}</span>
                                      <span v-else class="border-b-2">&emsp;&emsp;&emsp;&emsp;&emsp;</span>
                                    </p>
                                  </li>

                                  <li class="flex items-start">
                                    <p class="ml-3 text-base font-medium text-gray-500">
                                      <span class="font-bold text-primary">Topic: </span>
                                      <span v-if="registration.topic_id">{{ registration.topic.name }}</span>
                                      <span v-else class="border-b-2">&emsp;&emsp;&emsp;&emsp;&emsp;</span>
                                    </p>
                                  </li>
                                  <li class="flex items-start">
                                    <p class="ml-3 text-base font-medium text-gray-500">
                                      <span class="font-bold text-primary">Timeslot Duration: </span>
                                            <span v-if="registration.timeslot_duration">
                                           <span v-if="registration.timeslot_duration==='morning'">Morning - (8 am - 12 pm)</span>
                                          <span v-else-if="registration.timeslot_duration==='afternoon'">Afternoon - (12 pm - 4 pm)</span>
                                          <span v-else-if="registration.timeslot_duration==='evening'">Evening - (4 pm - 9 pm)</span>
                                        </span>
                                         <span v-else class="border-b-2">&emsp;&emsp;&emsp;&emsp;&emsp;</span>
                                  ...
                                  <li class="flex items-start">
                                    <p class="ml-3 text-base font-medium text-gray-500">
                                      <span class="font-bold text-primary">Class Size: </span>
                                      <span>{{ registration.class_type === 'Small Group' ? 'Small Group' : 'Private' }}</span>
                                    </p>
                                  </li>
                                  <li class="flex items-start">
                                    <p class="ml-3 text-base font-medium text-gray-500">
                                      <span class="font-bold text-primary">Class Frequency: </span>
                                      <span>{{registration.class_per_week === 1 ? 'One per week' : 'Two per week' }}</span>
                                    </p>
                                  </li>
                                  <li class="flex items-start">
                                    <p class="ml-3 text-base font-medium text-gray-500">
                                      <span class="font-bold text-primary">Payment Schedule: </span>
                                      <span v-if="registration.billing_cycle === 1">Monthly</span>
                                      <span v-else-if="registration.billing_cycle === 3">Quarterly</span>
                                      <span v-else-if="registration.billing_cycle === 6">Semi Annual</span>
                                    </p>
                  ...
                  <div>
                    <div class="mt-24">
                        <div class="text-center">
                            <button type="submit" class="shadow-primary w-full md:w-max  px-8 py-2 text-sm border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 hover:text-white focus:outline-none ">
                                <span class="text-xl">Continue</span> <br>
                            </button>
                        </div>
                    </div>
                </div>
        ...
        <transition :name="slide_animation">
          <form v-if="step === 1" @submit.prevent="submitStepOne" class="divide-y divide-black-lighter-4">
            <div class="space-y-8 pb-5">
              <div class="pt-2">
                <div v-if="step === 1" class="mt-5">
                  <h4 class="text-center">Select Your Preferred Class Schedule:</h4>
                </div>

                <div class="mt-5 text-center" v-if="!allSlotsSelected">
                  <p class="text-red-500 "><span class="font-extrabold text-black">Select your class start date: </span>Please select {{this.registration.class_per_week === 1 ? 'one class' : 'two classes'}}  for the next step!</p>
                </div>
                ...
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 md:gap-2">
                  ...
                  <div class="space-y-1 max-h-96 overflow-y-auto px-0 md:px-10">
                    <div class="text-primary flex flex-col items-center justify-center md:flex-row" v-if="registration.selected_date">
                      <div> {{filters.day_date(registration.selected_date_raw) }}</div>
                      <div class="md:pl-2"> ({{ filters.format_time_zone(timezone)}})</div>
                    </div>
                    <div v-else>
                    ...
                    <span>
                      ...
                      <div class="grid grid-cols-3 gap-1 md:gap-4 text-center text-primary" v-if="available_time_slots && available_time_slots.length > 0">
                        <span class="p-1 md:p-3 md:mx-2">Time</span>
                        <span class="col-span-2 p-1 md:p-3 md:mx-2">Teacher</span>
                      </div>

                    <div class="grid grid-cols-3 cursor-pointer gap-1 md:gap-4" v-for="(slot, index) in available_time_slots" :key="index" v-if="registration.selected_date" @click="selectClassHandler(slot)">
                      <div class="text-info bg-info-lighter-4 p-1 md:p-3 md:mx-2 rounded-xl text-center focus:bg-primary focus:text-white w-full">{{slot.slot_display}}</div>
                      <div class="col-span-2 text-info bg-info-lighter-4 p-1 md:p-3 md:mx-2 rounded-xl text-center focus:bg-primary focus:text-white capitalize w-full" >{{slot.teacher.full_name}}</div>
                    </div>
                  </div>
                </div>
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 md:gap-2">
                    <div v-for="(slot,index) in registration.selectedTaskClasses" :key="index">
                      <div class="flex items-center px-0 md:px-10">
                        <div class="w-full m-1 cursor-pointer space-y-2 md:space-y-0 md:space-x-2 inline-flex items-center justify-center px-2 py-2 text-sm border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none ">
            ...
            <div class="pt-10">
              <div class="flex justify-center">
                <button type="submit" :disabled="buttonLoader || !allSlotsSelected" class="disabled:opacity-50 disabled:cursor-not-allowed shadow-primary w-full md:w-max px-20 py-2 text-sm border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none ">
                  <span>Next</span>
                </button>
              </div>
              ...
              <div class="flex md:flex-col items-center justify-center space-x-3 py-3 pt-10 text-sm">
                ...
                <p class="underline">
                  <a  :href="'/contact'">Send a custom request to the team</a>
                </p>
              </div>
            </div>
          </form>
        ...
        <transition :name="slide_animation">
          <form class="md:px-36" v-if="step === 2" @submit.prevent="submitStepTwo">
            <div class="space-y-8">
              <div class="pt-2">
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6">
                  <div class="">
                    <label class="text-sm text-gray-700 flex">Phone Number <span class="text-danger font-bold text-xl">*</span> </label>
                    <div class="mt-1">
                      <vue-tel-input
                        v-bind="bindProps"
                  ...
                  <div class="">
                    <label class="text-sm text-gray-700 flex">Email <span class="text-danger font-bold text-xl">*</span></label>
                    <div class="mt-1">
                      <input required v-model="registration.parent.email" placeholder="Email" type="email" class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                    </div>
                  </div>
                  ...
                  <div class="md:col-span-2" >
                    <fieldset class="mt-6" v-if="existingStudents.length">
                      ...
                      <div class="grid grid-cols-1 md:grid-cols-3 mt-4 space-y-2">
                        <div class="flex items-center" v-for="student in existingStudents" :key="student.id">
                          <input type="radio" name="student" :value="student" v-model="registration.student" class="focus:ring-primary h-4 w-4 text-primary border-gray-300" required>
                          <label class="ml-3 block text-sm text-gray-700 capitalize">{{student.full_name}}</label>
                        </div>
                      </div>
                    </fieldset>
                  ...
                  <div class="md:col-span-2 text-info" v-if="existingStudents.length">
                    <div class="flex items-center justify-end space-x-3">
                      ...
                      <div class="" v-if="registration.addNewStudent">
                        <label class="text-sm text-gray-700 flex">Student Name <span class="text-danger font-bold text-xl">*</span></label>
                        <div class="mt-1">
                        <input type="text" placeholder="Student Name" v-model="registration.student.full_name" required class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                        </div>
                    </div>

                    <div class="" v-if="registration.addNewStudent">
                        <label class="text-sm text-gray-700 flex">Age <span class="text-danger font-bold text-xl">*</span></label>
                        <div class="mt-1">
                        <input type="number" placeholder="Age" v-model="registration.student.age" required class="shadow-sm focus:ring-primary focus:border-primary block w-full sm:text-sm border-gray-100 bg-gray-100 rounded-full">
                        </div>
                    </div>

                  <div class="md:col-span-2 pt-5" v-if="!registration.student.id">
                    <label class="text-sm text-gray-700 flex">How did you hear about us? <span class="text-danger font-bold text-xl">*</span></label>
                    <div class="mt-1">
                      <input
            ...
            <div class="pt-1">
              <div class="flex justify-center">
                <button type="submit" :disabled="buttonLoader" class="disabled:opacity-50 disabled:cursor-not-allowed shadow-primary w-full md:w-max px-20 py-2 text-sm font-medium border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none ">
                  <span>Next</span>
                </button>
              </div>
            </div>
        ...
        <transition :name="slide_animation">
          <section v-if="step === 3">
            <div class="space-y-20">
              <h5 class="text-center">{{registration.class_per_week === 1 ? 'Are you sure you want to book this class?' : 'Are you sure you want to book these classes?' }}</h5>
              <div class="md:pl-72 space-y-2">
                <p><span class="font-bold">Classes Selected ({{ filters.format_time_zone(timezone)}}) :</span> </p>
                <div class="p-1 py-2 my-2">
                  <div v-for="(slot,index) in registration.selectedTaskClasses" class="pb-2" :key="index">
                    <p><span class="font-bold"><span class="text-primary">Class - </span>{{ index+1 }}</span></p>
                    <p><span class="font-bold"><span class="text-primary">Teacher:</span></span> <span class="capitalize">{{slot.teacher.full_name}}</span> </p>
                    <p><span class="font-bold"><span class="text-primary">Class:</span></span> {{filters.day_date(slot.date)}} </p>
                    <p><span class="font-bold"><span class="text-primary">Time:</span></span> {{ slot.slot_display }}   </p>
                    <p><span class="font-bold"><span class="text-primary">Status:</span></span> Recurring weekly   </p>
                  </div>
                </div>
                <p><span class="font-bold"><span class="text-primary">Amount:</span></span> ${{total_amount}} </p>
                <p><span class="font-bold"><span class="text-primary">Location:</span></span> {{registration.location.location_name}} </p>
                <p><span class="font-bold"><span class="text-primary">Student Name:</span></span> <span class="capitalize">{{registration.student.full_name}}</span> </p>
                <p><span class="font-bold"><span class="text-primary">Email:</span></span> {{registration.parent.email}} </p>
                <p><span class="font-bold"><span class="text-primary">Phone Number:</span></span> {{phoneObject.countryCallingCode}} {{registration.parent.phone_number}} </p>
              </div>

              <div class="flex items-center justify-center text-center pt-20">
                <input required type="checkbox" v-model="tc" name="student" class="focus:ring-primary h-4 w-4 text-primary border-gray-300 bg-gray-100 rounded">
                <label class="ml-3 block text-sm text-gray-400 mt-1">
                  I agree to the
                  <a :href="drive_link" class="text-primary hover:text-primary-darker-1" target="_blank">terms of service, billing and privacy policy</a>
                  of Code With Us
                </label>
              </div>
              <div class="text-center space-y-2">
                <button @click="redirectToCheckout" :disabled="!tc" type="button" :class="!tc? 'disabled:opacity-50 disabled:cursor-not-allowed':''" class=" w-full md:w-52 shadow-primary inline-flex items-center justify-center px-10 py-2 text-sm border border-primary rounded-full text-white bg-primary hover:bg-primary-darker-1 focus:outline-none ">
                  <span>Book Class</span>
                  <!-- <Loader v-if="buttonLoader" variants="animate-spin" /> -->
                </button>
                <button @click="back" type="button" class="w-full md:w-52 shadow-primary inline-flex items-center justify-center px-10 py-2 text-sm border border-primary rounded-full text-primary hover:bg-primary hover:text-white focus:outline-none ">
                  Cancel
                </button>
              </div>
            </div>
            ...
            <div class="space-y-2">
              ...
              <div class="p-1 py-2 my-2">
                <div v-for="(slot,index) in registration.selectedTaskClasses" class="pb-2" :key="index">
                  <p><span class="font-bold"><span class="text-primary">Teacher:</span></span> <span class="capitalize">{{slot.teacher.full_name}}</span> </p>
                  <p><span class="font-bold"><span class="text-primary">Class:</span></span> {{filters.day_date(slot.date)}} </p>
                  <p><span class="font-bold"><span class="text-primary">Time:</span></span> {{ slot.slot_display }}, ({{ filters.format_time_zone(timezone)}})   </p>
                  <p><span class="font-bold"><span class="text-primary">Status:</span></span> Recurring weekly   </p>
                </div>
              </div>
              <p><span class="font-bold"><span class="text-primary">Location:</span></span> {{registration.location.location_name}} </p>
              ...
              <p><span class="font-bold"><span class="text-primary">Email:</span></span> {{registration.parent.email}} </p>
              <p><span class="font-bold"><span class="text-primary">Phone Number:</span></span> {{phoneObject.countryCallingCode}} {{registration.parent.phone_number}} </p>
              <p class="pt-10 text-primary font-extrabold"><a href="/how-to-videos" target="_blank">Please click here to learn how to access your weekly class</a></p>
            </div>
          </section>
        </transition>
    </div>
...
export default {
    ...
    computed : {
        ...
        sub_amount() {
            return parseFloat(this.price_per_class * this.registration.class_per_month * this.registration.class_per_week * this.registration.billing_cycle) // 1440
        },
        discount_extra() {
            let discount = 0;
            if (this.registration.class_per_week > 1) {
                discount = parseFloat((this.discount_two_class_per_week / 100) * this.sub_amount) // 144
            }
            return discount;
        },
        ...
        net_price() {
            return Math.floor((this.sub_amount - this.discount_extra - this.billing_cycle_discount)); // 1440 - 144 - 144
        },
        discount() {
            let discount = 0;
    ...
    methods  : {
        ...
        handleBillingCycleChange(){
            // console.log('billing cycle changed', this.registration.billing_cycle)
            this.getAvailableTimeSlots();
        ...
        submitStepTwo() {
            let _this = this;
            //console.log(this.registration,1,_this.phoneObject);
            ...
            //console.log(this.registration,2);
        ...
        fetchExistingStudents(){
            ...
            if (this.registration.phone.nationalNumber) {
                //console.log(this.registration.parent);
                this.studentStore.getExistingStudentsNew(this.registration).then((response) => {
                    //console.log(this.registration.parent.phone_number);
        ...
        switchPlan(planValue, event = null) {
            ...
            // if (!skipCallback && typeof this.handleBillingCycleChange === 'function') {
            //     this.handleBillingCycleChange();
            // }
        },
